...
def start (host=None,port=None):
    ...
    if current_connection == None: 
        ...
        if port == None and os.path.isfile(portfile):
            with open(portfile, 'r') as f:
                try:
                    ...
                    print("Problem reading ACT port number from",portfile,". Using default or 2650.")
                    port = 2650
        elif port == None:
            print("ACT port number file",portfile,"not found. Using default or 2650.")
            port = 2650
        ...
        if host == None and os.path.isfile(hostfile):
            with open(hostfile, 'r') as f:
                try:
                    ...
                    print("Problem reading ACT host from",hostfile,". Using default of 127.0.0.1.")
                    host = "127.0.0.1"
        elif host == None:
            print("ACT host file",hostfile,"not found. Using default of 127.0.0.1.")
            host = "127.0.0.1"
        ...
        try:
            ...
            print("Failed to connect to ACT with exception",sys.exc_info())
         
        else:   
            if a.interface.connected :
                a.interface.send("set-name","ACT Tutorial Python interface")
                current_connection = a
                return current_connection
            else:
                print("ACT connection NOT established, but no exception detected or already handled.")
    else:
        print("ACT is already connected.")
        return current_connection
...
def connection ():
    ...
    if current_connection == None:
        s = start()
        if s :
            print("ACT connection has been started.")
            return s
        else:
            print("Could not start ACT connection.")
    else:
        return current_connection
...
def stop():
    ...
    if current_connection == None:
        print("No current ACT connection to stop.")
    else:
        print("Closing down ACT connection.")
...
class interface():
    def __init__(self,host,port):
        ...
        try:
            ...
            print("Error trying to connect to ACT at",host,":",port,"with exception",sys.exc_info())
        else:
            self.connected = True
            self.cmd_id = 1
            self.actions = {}
    ...
    def collect_data(self):
        ...
        while c:
            try:
                ...
                if self.connected:
                    print("ACT connection error connection no longer available.")
                c = False
